(function () {
  ...
  function showPayPrompt(outcome) {
    ...
    const body = outcome === "lost"
      ? "Missed it this time — but that's exactly what whoever you send this to will play, with unlimited retries for them, not just one shot."
    ...
    overlay.innerHTML = `
      <div class="modal-box">
        <h2>${heading}</h2>
        <p>${body}</p>
        <a href="/payment/${templateId}" class="btn btn-primary wash" style="width: 100%; display: block; margin-top: 4px;">Make this for real — ${templatePrice} →</a>
